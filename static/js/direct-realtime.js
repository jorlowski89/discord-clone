(() => {
    const form = document.querySelector("[data-direct-composer]");
    ...
    if (!conversationId || !textarea) {
        return;
    }

    const protocol = window.location.protocol === "https:" ? "wss" : "ws";
    const socket = new WebSocket(`${protocol}://${window.location.host}/ws/dm/${conversationId}/`);
    ...
    const appendMessage = (message) => {
        ...
        if (message.image_url) {
            ...
            image.alt = "Obraz z wiadomości prywatnej";
            body.append(image);
        }
